...
import { ArrowRight, Bookmark, BriefcaseBusiness, CalendarCheck, ChevronDown, CircleUserRound, FileSearch, RefreshCw, ShieldCheck, Sparkles, Target } from "lucide-react";
  ...
  return <main className="appShell dashboardShell"><AppNav light />
    ...
    {data && <section className="dashboardGrid">
      ...
      <article className="dashboardPanel resumeProfilePanel"><div className="panelHeading"><div><span className="kicker">Private resume profile</span><h2>{data.resumeProfile ? data.resumeProfile.headline : "Add your resume intelligence"}</h2></div><Link href="/resume">{data.resumeProfile ? "Analyze again" : "Upload resume"}</Link></div>
        {data.resumeProfile ? <><p>{data.resumeProfile.summary}</p>{data.resumeDocument && <div className="resumeIntelligenceStats"><div><strong>{data.resumeDocument.wordCount.toLocaleString()}</strong><span>words securely indexed</span></div><div><strong>{data.resumeDocument.skills.length}</strong><span>evidenced skills</span></div><div><strong>{data.resumeDocument.experience.length}</strong><span>experience entries</span></div><div><strong>{Math.round(data.resumeDocument.extractionConfidence * 100)}%</strong><span>extraction confidence</span></div></div>}<dl><div><dt>Experience</dt><dd>{data.resumeProfile.yearsExperience} years · {data.resumeProfile.seniority}</dd></div><div><dt>Target roles</dt><dd>{data.resumeProfile.targetRoles.join(", ") || "Not detected"}</dd></div></dl><div className="skillTags">{(data.resumeDocument?.skills.map(skill => skill.name) || data.resumeProfile.skills).slice(0, 18).map(skill => <span key={skill}>{skill}</span>)}</div>{data.resumeDocument?.experience.length ? <div className="resumeTimeline"><h3>Experience extracted from your document</h3>{data.resumeDocument.experience.slice(0, 4).map((item, index) => <div key={`${item.company}-${item.title}-${index}`}><i/><span><b>{item.title || "Role"}</b><small>{item.company}{item.startDate || item.endDate ? ` · ${item.startDate || "?"}–${item.current ? "Present" : item.endDate || "?"}` : ""}</small></span></div>)}</div> : null}</> : <div className="panelEmpty"><ShieldIcon/><h3>Build your private resume intelligence</h3><p>Your original document, extracted text, and structured career JSON are encrypted for your account.</p><Link href="/resume">Analyze securely</Link></div>}
      </article>
      {data.resumeJobs?.length ? <article className="dashboardPanel resumeMatchPanel"><div className="panelHeading"><div><span className="kicker">Evidence-ranked opportunities</span><h2>Jobs matched to your resume</h2></div><Link href="/jobs">View all jobs</Link></div><div className="resumeMatchRows">{data.resumeJobs.map(job => <Link href={`/jobs/${job.id}`} key={job.id}><span className="companyLogo">{job.logo}</span><div><b>{job.title}</b><small>{job.company} · {job.matchConfidence} confidence</small><p>{job.matchReason}</p></div><strong>{job.fitScore}%</strong></Link>)}</div></article> : null}
      ...
      <aside className="dashboardPanel profilePanel"><CircleUserRound size={34}/><h2>{data.profile.name}</h2><p>{data.profile.email}</p><div><span>Profile strength <b>{data.profile.completion}%</b></span><progress max="100" value={data.profile.completion}/></div>{data.resumeDocument ? <><h3>Resume data health</h3><ul><li><CheckDot/>{data.resumeDocument.sectionsDetected.length} sections detected</li><li><CheckDot/>{data.resumeDocument.keywords.length} searchable keywords</li><li><span className="checkDot"><ShieldCheck size={13}/></span>Encrypted source retained</li></ul>{data.resumeDocument.warnings.length > 0 && <div className="dataReview"><FileSearch/><span><b>Review recommended</b><small>{data.resumeDocument.warnings[0]}</small></span></div>}</> : <><h3>This week</h3><ul><li><CheckDot/>Upload your latest resume</li><li><CheckDot/>Save 3 best-fit roles</li><li><CheckDot/>Practice your career story</li></ul></>}</aside>
    </section>}
  </main>;
}
