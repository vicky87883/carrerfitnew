...
export default function JobsPage() {
  ...
  return <main className="appShell">
    ...
    <section className="jobsWorkspace">
      ...
      <div className="results"><div className="resultsToolbar"><label><Search size={20}/><input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search title, company, or skill" /></label><span>{loading ? "Finding roles…" : `${jobs.length} roles found`}</span><Link className="sourceManagerLink" href="/job-sources"><Database/> Manage sources</Link></div>
