...
export default function JobDetailPage() {
  ...
  return <main className="appShell"><AppNav light />{error && <div className="emptyState">{error}</div>}{!job && !error && <div className="loadingPanel">Loading role…</div>}{job && <>
    <section className="jobDetailHero"><Link href="/jobs"><ArrowLeft size={17}/> All jobs</Link><div className="detailTitle"><span className="companyLogo large">{job.logo}</span><div><span className="fitPill"><Sparkles size={14}/>{job.fitScore}% match</span><h1>{job.title}</h1><p><Building2 size={17}/>{job.company}</p></div></div><div className="detailMeta"><span><MapPin/>{job.location}</span><span><Clock3/>Posted {job.postedDaysAgo} days ago</span><span><CircleDollarSign/>₹{job.salaryMin}–{job.salaryMax} LPA</span></div></section>
